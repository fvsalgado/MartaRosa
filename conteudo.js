...
/* Pré-visualização num ficheiro só: sem rede, os embeds ficariam caixas
   brancas. Nesse caso mostra-se o desenho da caixa em vez do leitor. */
const semRede = () => document.documentElement.dataset.semRede === '1';
...
function video(disco) {
  ...
  if (!com) return;
  if (semRede()) {
    cx.innerHTML = '<div class="ph"><div class="ph__c"><p class="ph__d num">vídeo oficial · O Músico</p></div></div>';
    return;
  }
